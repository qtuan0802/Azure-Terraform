...
resource "azurerm_key_vault_secret" "sapsfbatch_secrets" {
  for_each        = var.sapsfbatch_secrets
  name            = each.key
  value           = each.value
  key_vault_id    = azurerm_key_vault.main.id
  # expiration_date = timeadd(plantimestamp(), var.expiration_hours)
  expiration_date = timeadd(timestamp(), format("%dh", var.expiration_hours))
  depends_on = [
    azurerm_key_vault_access_policy.current_user
  ]
}
